// Vercel serverless webhook for Stripe payment events (localloop)
...
const GH_REPO = process.env.GH_DATA_REPO || 'localloop-data';
const MAIL_FROM = process.env.LOCALLOOP_MAIL_FROM || 'gentledesk632@agentmail.to';
const AGENTMAIL_KEY = process.env.AGENTMAIL_API_KEY || '';
const BUYERS_FILE = 'buyers.json';
const USERS_FILE = 'users.json';
const ACCESS_PEPPER = process.env.ACCESS_PEPPER || 'localloop-pepper-7q4w2z';

function genCode() {
  const chars = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
  let s = '';
  for (let i = 0; i < 10; i++) s += chars[crypto.randomInt(chars.length)];
  return `LL-${s.slice(0, 5)}-${s.slice(5)}`;
}
...
async function sendEmail(buyer, code) {
  ...
  const body =
`Welcome to LocalLoop™ — The LocalLoop™ Private AI System ✅

Hi ${buyer.name || 'there'},

Thank you! Your order is confirmed.

YOUR PERSONAL ACCESS CODE: ${code || 'LL-XXXXX-XXXXX'}

OPEN YOUR DOWNLOADS:
→ https://localloop.vercel.app/download.html
(enter your email + access code above)

WHAT'S INSIDE (8 deliverables):
• 1. The LocalLoop System — your first private AI, in 7 evenings
• 2. The Hardware Tiers — what actually runs on your machine
• 3. The Model Picker — choose a model by job, not hype
• 4. The Context Pack — the 5 files that make it sound like you
• 5. The Voice Contract — a voice that doesn't drift (+ anti-slop ruleset)
• 6. The Loop Builder — make it run on its own (schedules + kill switch)
• 7. The Run Log & Trust Gate — prove it ran, then promote it safely
• 8. The LocalLoop Playbook — starter scripts, 30-day roadmap, $0 launch

START TONIGHT:
1. Read The LocalLoop System (10 minutes) — the four-stage model
2. Install a free runtime and run one small model (Day 1)
3. Write your Context Pack this week — then build your first loop

60-day, no-questions-asked "Peace of Mind" guarantee — keep everything either way.

Stop paying rent on your own brain.
— The LocalLoop Team`;
const res = await fetch(`https://api.agentmail.to/v0/inboxes/${MAIL_FROM}/messages/send`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${AGENTMAIL_KEY}` },
    body: JSON.stringify({ to: [buyer.email], subject: "Your LocalLoop™ Private AI System access ✅", text: body }),
  });
  return res.ok;
}
